...
errors quote both values (`read back "" after writing "千代田区"`); when every
...
Examples:
  ...
  chrome-use type @e5 "100-0001" --key-events   # trigger the address autocomplete
...
Agent loop (experimental):
  jev run --goal <text> [--url <url>] [--terminal-shadow]
                             ...
                             the request sent, the candidates shown, the choice,
                             and Jev's answer probabilities. Off unless set. It
                             records the goal, page text and field values, so
