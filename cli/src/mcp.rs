...
fn extended_tools() -> Vec<Value> {
    vec![
        ...
        json!({
            "name": TOOL_TABS,
            "description": "List browser tabs with ownership, open or duplicate tabs, explicitly adopt existing tabs, inspect tabs, close session-created tabs, or switch to created/adopted tabs.",
            "inputSchema": build_schema(obj(&[
                ...
                ("tabId", json!({ "type": "string", "description": "Tab id, label, or stable target ID. Switch accepts created/adopted tabs; close accepts only session-created tabs. Required for action=switch or inspect; optional for action=close or duplicate (defaults to current tab)." })),
