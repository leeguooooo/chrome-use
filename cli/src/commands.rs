...
mod tests {
    ...
    fn test_type_key_events() {
        ...
        let cmd = parse_command(
            &args("type #postal 100-0001 --key-events"),
            &default_flags(),
        )
        .unwrap();
        assert_eq!(cmd["action"], "type");
        assert_eq!(cmd["selector"], "#postal");
        assert_eq!(cmd["text"], "100-0001");
        assert_eq!(cmd["keyEvents"], true);

        let focused =
            parse_command(&args("type --focused 100-0001 --keys"), &default_flags()).unwrap();
        assert_eq!(focused["focused"], true);
        assert_eq!(focused["text"], "100-0001");
