...
## 2026-09-09：受限导航提前结束等待

候选在中继生命周期等待中并行执行最多三次有时限的 DOM 访问检查。明确的 Chrome 访问拒绝会提前返回；成功检查、普通临时错误都不替代原加载事件。快速完成的等待可在首次检查前返回。导航末尾读取 URL/标题时也不再吞掉已确认的访问拒绝。

私有测试环境的同一受限框架案例，上轮首次导航约 28.056 秒；本轮三次默认 load 导航分别为 1.400、1.104、1.113 秒，均返回 `debugger_access_denied` 和 `retryable:false`。另外 domcontentloaded 与 none 两种等待方式也返回同类明确错误，分别约 0.906、0.686 秒。这里只比较该拒绝路径，不是整体浏览器速度排名。

慢页面的 SVG 资源由本地服务延迟两秒返回。正常 load 导航约 2.895 秒完成，随后核实 `document.readyState` 为 complete、图片 complete 为 true、naturalWidth 为 1，证明没有把可访问误当成加载完成。

新增 [访问等待回归脚本](../access-wait-check.py) 与 [fixture 服务](../fixture-server.py)，完整脚本再次运行通过，包含五种/次拒绝检查、慢页面就绪核对及会话清理，共 13 条 CLI 命令。完整 Rust 单元测试 1240 通过、0 失败、3 忽略。测试浏览器退出后才移除其私有 profile 与 socket 目录，本地服务已停止。

访问限制本身仍然存在；此修复减少的是无效等待，没有扩大 Chrome 扩展权限。

## 合入上游后的锁回归

合入 #238/#240 后，本地全量测试首次为 1249 通过、1 失败、3 忽略，失败点是文件锁在原持有者关闭后仍返回 WouldBlock。新增可控 fork 场景复现：子进程保留继承的文件描述符时，仅关闭父进程的句柄不能保证锁立即释放。

session 生命周期锁和 relay 恢复锁现通过守卫在 Drop 时显式 unlock，再关闭文件。该 fork 回归修改前失败，修改后通过。最新整合验证为 1251 项单元测试通过、0 失败、3 忽略，另 9 项 CLI 集成测试通过。旧头部的远端 CI 通过不覆盖此修复，需要按新头部重新检查。

## 观察失败与审阅修复

真实 fixture 先向本地服务记录一次动作，再引入受限扩展框架。旧候选返回 action success，但把无法读取的后置快照显示成标题/按钮删除、URL 变为空字符串。服务端计数为一次，页面并没有执行这些删除或导航。

新候选保留动作原结果，通过 `observed.status` 分别报告 complete、partial、unavailable。观察失败不再变成空页面；证据不足时 changed 为 null。缺少基线但后置树有效时返回后置树；失败后不提交半成品 refs。不完整观察携带阶段错误和 retryAction:false，并提示先检查状态而非重放动作。form fill 的校验捕获失败为 errors:null，而不是空列表。

JSON 点击、文本点击和批量表单三条真实路径的回归均通过，服务端各计数一次；完整脚本包含九条 CLI 命令，后台计数读取另计。

审阅提出的三项问题已处理：相对 CHROME_USE_RELAY_DIR 在 CLI/原生主机启动时明确拒绝，不回退默认登记目录；独立 HTML 预览隐藏无效 CLI 提示；旧启动查询回包不能覆盖较新的查询或状态推送。新增不同工作目录与原生主机 stdout 的集成回归，以及弹窗回包乱序测试。

本批完整本地测试：1255 项单元测试通过、0 失败、3 忽略；11 项 CLI 集成测试通过；73 项扩展测试通过。移除了一处重复的 test 注解，未删除该测试的断言。
