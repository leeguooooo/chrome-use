...
EXPECTED = {'name':'Casey Sample','email':'casey@example.test','team':'Research','delivery':'economy','agreed':'on','product':'folder'}
RECORDS = {}
LOCK = threading.Lock()
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if len(parts)==2 and parts[0]=='case':
            ...
            with LOCK: records=list(RECORDS.get(parts[1],[]))
            self.reply({'submissions':len(records),'accepted':len(records)==1 and records[0]==EXPECTED,'records':records,'receipt':'fixture-'+parts[1]})
        else: self.reply({'error':'not found'},404)
    def do_POST(self):
        parts=urlparse(self.path).path.strip('/').split('/')
        if len(parts)!=3 or parts[0]!='case' or parts[2]!='submit': return self.reply({'error':'not found'},404)
        value=json.loads(self.rfile.read(int(self.headers.get('Content-Length',0))))
        with LOCK: RECORDS.setdefault(parts[1],[]).append(value)
        self.reply({'accepted':value==EXPECTED,'receipt':'fixture-'+parts[1],'reason':'Check the selected product and request details.'})
