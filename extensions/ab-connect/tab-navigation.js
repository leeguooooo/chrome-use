...
export async function navigateTabWithBrowserFallback(params, deps) {
  // Used as given. The CLI already adds a scheme to a bare host before it gets
  // here (commands.rs); #342 also rewrote it in the extension, which could only
  // ever fire for other callers and mangled schemes without `//` — `blob:`,
  // `view-source:` and `mailto:` became `https://blob:...`.
